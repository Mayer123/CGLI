...
def readPredictions(fnPred, labels):
    ret = {}
    mapping = {'lifted vapor': 'lift vapor',  'alternating current':'alternate current', 'air masses': 'air mass', 
    'masses':'mass', 'warm, moist air': 'warm , moist air', 'cool, dry air':'cool , dry air',
    'dry, cold high-pressure air':'dry , cold high-pressure air', 'chunks of rock':'chunk of rock',
    'deoxygenated blood':'deoxygenate blood', 'oxygenated blood':'oxygenate blood', 'animal\'s skeleton':'animal \'s skeleton',
    'new types of rocks': 'new type of rock', 'certain types of rocks': 'certain type of rock',
    'bones and hard tissues': 'bone and hard tissue', 'retina\'s rods and cones': 'retina \'s rod and cone',
    'living things': 'live thing', 'remains of living things': 'remains of live thing'}
    for f in fnPred: 
        
        pid, sid, participant_, from_location, to_location = int(f[0]), int(f[1]), f[2], f[3], f[4]
        gold_participants = findAllParticipants(labels[pid])
        for p in participant_.split(';'):
            participant = p.strip()
            if participant not in gold_participants:
                if participant.endswith('s') and participant[:-1] in gold_participants:
                    participant = participant[:-1]
                    
                elif participant.lower() in gold_participants:
                    participant = participant.lower()
                elif participant in mapping and mapping[participant] in gold_participants:
...
def print_metrics(fnPred):
    qid_to_score = {}

    selPid = set([int(x) for x in open('../data/Evalfiles/para_id.test.txt').readlines()])
    gold_labels = readGold('../data/Evalfiles/gold_labels.test.tsv')
    labels = readLabels('../data/Evalfiles/all-moves.full-grid.tsv', selPid, gold_labels)
    predictions = readPredictions(fnPred, labels)
    ...
    return round(cat1_score,2), round(cat2_score,2), round(cat3_score,2), round(macro_avg,2), round(micro_avg,2)
...
def main():
    if len(sys.argv) != 2:
        sys.stderr.write("Usage: evalQA.py system-predictions\n")
        sys.exit(-1)
    fnPred = sys.argv[1]
    qid_to_score = {}
    selPid = set([int(x) for x in open('../data/Evalfiles/para_id.test.txt').readlines()])
    ...
    pred_data = []
    swap = {'-': 'null', '?': 'unk'}
    with open(fnPred, 'r') as f:
        for line in f:
            line = line.strip().split('\t')
            line = [line[0], line[1], line[2], line[4], line[5]]
            pred_data.append([w if w not in swap else swap[w] for w in line])
    print (pred_data[:5])
    predictions = readPredictions(pred_data, labels)
